...
io.on('connection', (socket) => {
  ...
  socket.on('chat_message', (data) => {
    ...
    if (role === 'user') {
      for (let admin of admins) admin.emit('chat_message', msg);
    }
    if (role === 'admin') {
      const target = io.sockets.sockets.get(msg.user_id);
      if (target) target.emit('chat_message', msg);
    }
  });

  socket.on('select_user', (uid) => {
    if (role === 'admin') socket.emit('history', chats[uid] || []);
  });
  ...
  socket.on('delete_history', (uid) => {
    if (role === 'admin') {
      ...
      if (target) target.emit('history_deleted', { user_id: uid });
      for (let admin of admins) admin.emit('history_deleted', { user_id: uid });
    }
  });
